#!/usr/bin/env python3
# producer.py - Interactive REPL to inject prompts into ai_in topic
...
log_file = os.path.join(log_dir, 'hippocampus-producer.log')
...
class PromptProducer:
    def __init__(self,
        ...
        self.bootstrap_servers = bootstrap_servers or config.get('KAFKA_BOOTSTRAP_SERVERS', 'localhost:9092')
        self.topic = topic or config.get('ENRICHENER_INPUT_TOPIC', 'ai_in')
        self.send_timeout = int(send_timeout or config.get_int('KAFKA_SEND_TIMEOUT', 10))
    ...
    def send_prompt(self, text):
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                ...
                logger.debug(f"Waiting for confirmation (timeout: {self.send_timeout}s)...")
                ...
                logger.warning("="*60)
                logger.warning(f"TIMESTAMP ERROR on attempt {attempt + 1}/{self.max_retries}")
                logger.warning("="*60)
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.error("="*60)
                    logger.error(f"FAILED after {self.max_retries} attempts (timestamp issues)")
                    logger.error("="*60)
                    return False
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.error("="*60)
                    logger.error(f"FAILED after {self.max_retries} attempts (Kafka error)")
                    logger.error("="*60)
                    return False

            except Exception as e:
                logger.error("="*60)
                logger.error(f"UNEXPECTED ERROR sending prompt")
                logger.error("="*60)
    ...
    def start_repl(self):
        ...
        history_file = os.path.expanduser('~/.hippocampus_producer_history')
        ...
        print("\n" + "="*60)
        print("Hippocampus Prompt Producer REPL")
        print("="*60)
        print(f"Sending prompts to topic: {self.topic}")
        print("Prompts will be enriched with context before AI processing")
...
if __name__ == "__main__":
    ...
    logger.info(f"  KAFKA_BOOTSTRAP_SERVERS: {config.get('KAFKA_BOOTSTRAP_SERVERS', 'not set')}")
    logger.info(f"  ENRICHENER_INPUT_TOPIC: {config.get('ENRICHENER_INPUT_TOPIC', 'not set')}")
