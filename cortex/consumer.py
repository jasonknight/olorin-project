...
class CortexConfig:
    ...
    def _load(self):
        ...
        self.kafka_consumer_group = self.cfg.get('CORTEX_CONSUMER_GROUP', 'exo-consumer-group')
        self.kafka_auto_offset_reset = self.cfg.get('CORTEX_AUTO_OFFSET_RESET', 'earliest')
        ...
        # Default path is relative to project root (cortex runs from cortex/)
        default_context_db = os.path.join(os.path.dirname(__file__), '..', 'hippocampus', 'data', 'context.db')
        self.context_db_path = self.cfg.get('CONTEXT_DB_PATH', default_context_db)
        self.cleanup_context_after_use = self.cfg.get_bool('CLEANUP_CONTEXT_AFTER_USE', True)

        # Chat history settings
        default_chat_db = os.path.join(os.path.dirname(__file__), 'data', 'chat.db')
        self.chat_db_path = self.cfg.get('CHAT_DB_PATH', default_chat_db)
        self.chat_history_enabled = self.cfg.get_bool('CHAT_HISTORY_ENABLED', True)

        # Reset command patterns (comma-separated list)
        reset_patterns_str = self.cfg.get('CHAT_RESET_PATTERNS', '/reset,reset conversation,new conversation,start over')
        self.chat_reset_patterns = [p.strip().lower() for p in reset_patterns_str.split(',')]
...
def load_config() -> CortexConfig:
    ...
    logger.info(f"  Model Name: {cortex_cfg.model_name if cortex_cfg.model_name else 'auto-detect'}")
    logger.info(f"  Temperature: {cortex_cfg.temperature}")
    logger.info(f"  Max Tokens: {cortex_cfg.max_tokens if cortex_cfg.max_tokens else 'use exo default'}")
...
def strip_markdown(text: str) -> str:
    ...
    text = re.sub(r'(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)', r'\1', text)  # Italic *text* (not part of **)
    text = re.sub(r'(?<!_)_(?!_)(.+?)(?<!_)_(?!_)', r'\1', text)        # Italic _text_ (not part of __)
...
class ExoConsumer:
    def __init__(self, config: CortexConfig):
        ...
        logger.info(f"Creating Kafka consumer for topic '{config.kafka_input_topic}'...")
        ...
        logger.info(f"Creating Kafka producer for output topic '{config.kafka_output_topic}'...")
        logger.info(f"  Bootstrap servers: {config.kafka_bootstrap_servers}")
        try:
        ...
        self.executor = ThreadPoolExecutor(max_workers=3, thread_name_prefix="msg-processor")
        self.active_futures = []
        self.shutdown_event = threading.Event()
        logger.info("Thread pool executor initialized with 3 workers")

        logger.info(f"ExoConsumer initialized successfully")
    ...
    def _init_openai_client(self, config: CortexConfig):
        ...
        client = OpenAI(
            base_url=config.exo_base_url,
            api_key=config.exo_api_key
        )
    ...
    def _init_chat_store(self):
        ...
        try:
            ...
            logger.info(f"Chat store initialized successfully")
    ...
    def _handle_reset_command(self, message_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"[{thread_name}] Conversation reset. New conversation ID: {new_conv_id}")
    ...
    def _get_running_model(self) -> str | None:
        ...
        try:
            ...
            logger.debug(f"[{thread_name}] Response status code: {response.status_code}")
            ...
            logger.info(f"[{thread_name}] Found {len(instances)} instance(s) and {len(runners)} runner(s) in exo state")

            if not instances:
                logger.warning(f"[{thread_name}] No instances in state - exo may not be running")
                return None
                ...
                if isinstance(instance_wrapper, dict):
                    ...
                    for variant_name, instance_data in instance_wrapper.items():
                        ...
                        if isinstance(instance_data, dict):
                            ...
                            logger.debug(f"[{thread_name}]   Runners: {list(runner_to_shard.keys())}")
                            ...
                            for runner_id in runner_to_shard.keys():
                                ...
                                logger.debug(f"[{thread_name}]   Runner {runner_id} status: {status_type}")
                                ...
                                if status_type not in ["RunnerReady", "RunnerRunning"]:
                                    ...
                                    logger.debug(f"[{thread_name}]   ✗ Runner {runner_id} not ready (status: {status_type})")
                                    break

                            if all_runners_ready and runner_to_shard:
                                logger.info(f"[{thread_name}] ✓ Auto-detected READY model: {model_id}")
                                logger.info(f"[{thread_name}]   All {len(runner_to_shard)} runner(s) are ready")
                                return model_id
                            elif model_id:
                                logger.info(f"[{thread_name}] ⏳ Found model {model_id} but runners not all ready yet")

            logger.warning(f"[{thread_name}] ✗ No ready model found in {len(instances)} instance(s)")
            return None
            ...
            logger.error(f"[{thread_name}] ✗ Failed to query exo state: {type(e).__name__}: {e}")
            return None
...
---"""
        ...
        assistant_ack = "I understand. I'll use this context to help answer your questions."
    ...
    def _build_messages_with_history(
        ...
        if self.chat_store is None:
            ...
            logger.info(f"[{thread_name}] Chat history disabled, built {len(messages)} message(s)")
            return messages
        ...
        try:
            ...
            if context_chunks:
                ...
                logger.info(f"[{thread_name}] Injected RAG context as user/assistant exchange")

            # Add current user prompt
            messages.append({"role": "user", "content": prompt})

            # Store the user message in chat history
            self.chat_store.add_user_message(conversation_id, prompt, prompt_id=message_id)

            logger.info(f"[{thread_name}] Built messages array with {len(messages)} total messages")
            logger.info(f"[{thread_name}]   - History: {len(history)}, Context exchange: {2 if context_chunks else 0}, New prompt: 1")
            return messages

        except Exception as e:
            logger.error(f"[{thread_name}] Error building message history: {e}", exc_info=True)
            ...
            messages.append({"role": "user", "content": prompt})
            logger.warning(f"[{thread_name}] Fallback: built {len(messages)} message(s) without history")
            return messages

    def _build_context_enriched_prompt(self, prompt: str, prompt_id: str) -> tuple[str, int]:
        """
        ...
        if self.context_store is None:
            logger.warning(f"[{thread_name}] Context store not available, using original prompt")
            return prompt, 0

        try:
            contexts = self.context_store.get_contexts_for_prompt(prompt_id)
            if not contexts:
                logger.info(f"[{thread_name}] No context found for prompt_id={prompt_id}")
                return prompt, 0

            logger.info(f"[{thread_name}] Retrieved {len(contexts)} context chunks for prompt_id={prompt_id}")
            ...
            for i, ctx in enumerate(contexts):
                ...
                source_ref = ' > '.join(source_parts) if source_parts else f"Chunk {i+1}"
                distance = ctx.get('distance')
                distance_str = f" (relevance: {1 - distance:.2%})" if distance is not None else ""

                context_parts.append(f"### {source_ref}{distance_str}\n{ctx['content']}")
...
User's question: {prompt}"""
            ...
            logger.info(f"[{thread_name}] Built enriched prompt with {len(contexts)} context chunks")
            logger.debug(f"[{thread_name}] Context block length: {len(context_block)} chars")

            return enriched_prompt, len(contexts)

        except Exception as e:
            logger.error(f"[{thread_name}] Error retrieving context: {e}", exc_info=True)
            return prompt, 0
    ...
    def _cleanup_context(self, prompt_id: str):
        ...
        if not self.config.cleanup_context_after_use:
            logger.debug(f"[{thread_name}] Context cleanup disabled, keeping context for prompt_id={prompt_id}")
            return

        try:
            deleted = self.context_store.delete_contexts_for_prompt(prompt_id)
            logger.info(f"[{thread_name}] Cleaned up {deleted} context chunks for prompt_id={prompt_id}")
        except Exception as e:
    ...
    def _check_config_reload(self):
        ...
        if self.config.reload():
            ...
            if self.config.model_name != old_model_name:
                ...
                new_model = self.config.model_name if self.config.model_name else 'auto-detect'
                logger.info(f"Model changed: {old_model} -> {new_model}")
            if self.config.temperature != old_temperature:
                logger.info(f"Temperature changed: {old_temperature} -> {self.config.temperature}")
            if self.config.max_tokens != old_max_tokens:
                old_tokens = old_max_tokens if old_max_tokens else 'exo default'
                new_tokens = self.config.max_tokens if self.config.max_tokens else 'exo default'
    ...
    def process_message(self, message):
        ...
        try:
            ...
            if isinstance(message, str):
                logger.info(f"[{thread_name}] Message is a string, attempting JSON parse...")
                try:
                    ...
                    logger.info(f"[{thread_name}]   - Context available: {context_available}")
                    ...
                    logger.info(f"[{thread_name}]   - Plain text prompt: '{prompt[:50]}...'")
            ...
            if not prompt:
                logger.warning(f"[{thread_name}] ✗ Empty prompt detected in message: {message}")
                logger.warning(f"[{thread_name}] SKIPPING message processing - no content to process")
                return

            logger.info(f"[{thread_name}] ✓ Message parsing complete")
            logger.info(f"[{thread_name}]   Message ID: {message_id}")
            logger.info(f"[{thread_name}]   Prompt preview: {prompt[:100]}{'...' if len(prompt) > 100 else ''}")
            ...
            if self._is_reset_command(prompt):
                ...
                if self._handle_reset_command(message_id):
                    logger.info(f"[{thread_name}] ✓ Conversation reset completed, skipping normal processing")
                    return
                logger.warning(f"[{thread_name}] ⚠ Reset handling failed, continuing with normal processing")

            # Step 2: Retrieve context chunks and build messages with history
            logger.info(f"[{thread_name}] STEP 2/6: Building messages with conversation history...")
            context_chunks = None
            context_count = 0
            if context_available and contexts_stored > 0:
                logger.info(f"[{thread_name}] 📚 Context available! Retrieving {contexts_stored} context chunks...")
                if self.context_store is not None:
                    context_chunks = self.context_store.get_contexts_for_prompt(message_id)
                    context_count = len(context_chunks) if context_chunks else 0
                    if context_count > 0:
                        logger.info(f"[{thread_name}] ✓ Retrieved {context_count} context chunks for RAG injection")
                    else:
                        logger.warning(f"[{thread_name}] ⚠ Context was indicated but none retrieved")
                else:
                    logger.warning(f"[{thread_name}] ⚠ Context store not available, skipping context retrieval")
            else:
                logger.info(f"[{thread_name}] No context enrichment needed (context_available={context_available})")

            # Build messages array with conversation history
            messages = self._build_messages_with_history(prompt, message_id, context_chunks)
            ...
            logger.info(f"[{thread_name}] 📣 Sending 'Processing, one moment...' notice to Broca")
            ...
            if not model_to_use:
                logger.info(f"[{thread_name}] No model in config, auto-detecting from exo...")
                logger.info(f"[{thread_name}] Querying exo state endpoint for running model...")
                model_to_use = self._get_running_model()
                if not model_to_use:
                    logger.error(f"[{thread_name}] ✗ Auto-detection failed: no running model found")
                    raise ValueError("No model specified in config and no running instances found in exo")
                logger.info(f"[{thread_name}] ✓ Auto-detected model: {model_to_use}")
            else:
                logger.info(f"[{thread_name}] ✓ Using model from config: {model_to_use}")
            ...
            logger.info(f"[{thread_name}]   Max tokens: {self.config.max_tokens if self.config.max_tokens else 'exo default'}")
            logger.info(f"[{thread_name}] ⏳ Waiting for AI inference (this may take a while)...")
            ...
            full_response_text = ""
            word_buffer = ""
            sentence_buffer = ""  # Accumulates complete sentences until word threshold is met
            chunk_count = 0
            chunks_sent = 0
            ...
            PAUSE_PUNCTUATION = {',', ';', ':'}  # Mid-sentence pauses (used for splitting within sentences)

            logger.info(f"[{thread_name}] 📦 Growing chunk sizes enabled:")
            logger.info(f"[{thread_name}]   Initial word threshold: {INITIAL_WORD_THRESHOLD}")
            logger.info(f"[{thread_name}]   Growth factor: {GROWTH_FACTOR} (50% increase per chunk)")
            logger.info(f"[{thread_name}]   Sentence boundaries: {SENTENCE_PUNCTUATION}")
            logger.info(f"[{thread_name}] 🧠 Tracking thinking blocks in real-time - will skip sending during <think>/<thinking> tags")
                ...
                if chunk.choices and len(chunk.choices) > 0:
                    ...
                    if hasattr(delta, 'content') and delta.content:
                        ...
                        if first_chunk_time is None:
                            ...
                            time_to_first_chunk = (first_chunk_time - api_start_time).total_seconds()
                            logger.info(f"[{thread_name}] ⚡ First chunk received in {time_to_first_chunk:.2f}s")

                        # Check for thinking block tags in this chunk
                        if '<think>' in content or '<thinking>' in content:
                            if not in_thinking:
                                logger.info(f"[{thread_name}] 🧠 Entered thinking block - pausing Kafka sends")
                                in_thinking = True
                                ...
                                logger.info(f"[{thread_name}] 💭 Sending 'Thinking...' notice to Broca")
                                self.producer.send(self.config.kafka_output_topic, value=thinking_message)

                        if '</think>' in content or '</thinking>' in content:
                            if in_thinking:
                                logger.info(f"[{thread_name}] 🧠 Exited thinking block - resuming Kafka sends")
                                in_thinking = False
                                ...
                                if earliest_pos != -1:
                                    ...
                                    if current_word_count >= current_word_threshold:
                                        ...
                                        if tts_chunk.strip():
                                            ...
                                            logger.info(f"[{thread_name}] 📤 Sending chunk #{chunks_sent} ({current_word_count} words >= threshold {int(current_word_threshold)}, {len(tts_chunk)} chars)")
                                            self.producer.send(self.config.kafka_output_topic, value=chunk_output)
                                            ...
                                            logger.info(f"[{thread_name}] 📈 Word threshold increased: {int(old_threshold)} → {int(current_word_threshold)} words")
            ...
            if remaining_content.strip() and not in_thinking:
                ...
                if tts_chunk.strip():
                    ...
                    logger.info(f"[{thread_name}] 📤 Sending FINAL chunk #{chunks_sent} ({final_word_count} words, {len(tts_chunk)} chars)")
                    self.producer.send(self.config.kafka_output_topic, value=chunk_output)
            ...
            logger.info(f"[{thread_name}]   Total duration: {api_duration:.2f} seconds ({api_duration/60:.1f} minutes)")
            logger.info(f"[{thread_name}]   Chunks received: {chunk_count}")
            logger.info(f"[{thread_name}]   Chunks sent to Kafka: {chunks_sent}")
            logger.info(f"[{thread_name}]   Full response length: {len(full_response_text)} characters")
            ...
            if thinking_blocks:
                ...
                logger.info(f"[{thread_name}]   Number of thinking blocks: {stats['block_count']}")
                logger.info(f"[{thread_name}]   Total words in thinking: {stats['total_words']}")
                logger.info(f"[{thread_name}]   Total characters in thinking: {stats['total_characters']}")
            ...
            if self.chat_store is not None:
                try:
                    ...
                    if conversation_id:
                        # Store the cleaned response (without thinking blocks)
                        self.chat_store.add_assistant_message(conversation_id, cleaned_text)
                        logger.info(f"[{thread_name}] 💬 Stored assistant response in chat history ({len(cleaned_text)} chars)")
                except Exception as e:
                    logger.warning(f"[{thread_name}] ⚠ Failed to store assistant response in chat history: {e}")

            logger.info("▲"*60)
            logger.info(f"[{thread_name}] ✓✓✓ MESSAGE PROCESSING COMPLETED SUCCESSFULLY ✓✓✓")
            logger.info(f"[{thread_name}]   Sent {chunks_sent} chunk(s) with growing word threshold ({INITIAL_WORD_THRESHOLD} → {int(current_word_threshold)} words)")
            if context_count > 0:
                logger.info(f"[{thread_name}]   Used {context_count} context chunks from hippocampus")
            if self.chat_store is not None:
                logger.info(f"[{thread_name}]   Chat history: messages stored for conversation continuity")
            logger.info("▲"*60)
            ...
            try:
                ...
                logger.info(f"[{thread_name}] ✓ Error message sent to Kafka successfully")
            except Exception as send_error:
                logger.error(f"[{thread_name}] ✗ Failed to send error message: {send_error}", exc_info=True)
            logger.error("✗"*60)
    ...
    def _process_message_wrapper(self, message_value, message_count, message_metadata):
        ...
        thread_name = threading.current_thread().name
        logger.info("="*80)
        logger.info(f"[THREAD WORKER] Thread {thread_name} (ID: {thread_id}) STARTED processing message #{message_count}")
        logger.info(f"[THREAD WORKER] Thread pool status: {len(self.active_futures)} active workers")
        logger.info(f"[THREAD WORKER] Message metadata: partition={message_metadata['partition']}, offset={message_metadata['offset']}")
        logger.info(f"[THREAD WORKER] Message value: {message_value[:100]}..." if len(str(message_value)) > 100 else f"[THREAD WORKER] Message value: {message_value}")
        logger.info("="*80)
        ...
        try:
            ...
            logger.info("="*80)
            logger.info(f"[THREAD WORKER {thread_name}] ✓ SUCCESSFULLY processed message #{message_count}")
            logger.info(f"[THREAD WORKER {thread_name}] Processing took {duration:.2f} seconds")
            logger.info("="*80)
        except Exception as e:
            end_time = datetime.now()
            duration = (end_time - start_time).total_seconds()
            logger.error("="*80)
            logger.error(f"[THREAD WORKER {thread_name}] ✗ FAILED to process message #{message_count}")
            logger.error(f"[THREAD WORKER {thread_name}] Error after {duration:.2f} seconds: {e}")
            logger.error(f"[THREAD WORKER {thread_name}] Error type: {type(e).__name__}")
    ...
    def start(self):
        ...
        try:
            ...
            logger.info("[MAIN THREAD] Main thread will continuously poll Kafka while workers process messages")
            logger.info("[MAIN THREAD] This prevents Kafka rebalancing during long-running AI inference")

            for message in self.consumer:
                # Check for shutdown signal
                if self.shutdown_event.is_set():
                    logger.warning("[MAIN THREAD] Shutdown event detected, stopping consumer loop")
                    break
                ...
                logger.info("\n" + "█"*80)
                logger.info(f"[MAIN THREAD] ▼▼▼ MESSAGE #{message_count} RECEIVED FROM KAFKA ▼▼▼")
                logger.info("█"*80)
                logger.info(f"[MAIN THREAD] Receive timestamp: {receive_timestamp.isoformat()}")
                logger.info(f"[MAIN THREAD] Kafka message details:")
                ...
                logger.info(f"[MAIN THREAD]   Value preview: {str(message.value)[:150]}...")
                logger.info(f"[MAIN THREAD]   Value length: {len(str(message.value))} characters")

                # Check if configuration has changed (quick operation, safe to do in main thread)
                logger.debug(f"[MAIN THREAD] Checking for configuration changes...")
                self._check_config_reload()
                logger.debug(f"[MAIN THREAD] Configuration check complete")

                # Get thread pool stats before submission
                active_count = len(self.active_futures)
                logger.info(f"[MAIN THREAD] Thread pool status BEFORE submission:")
                logger.info(f"[MAIN THREAD]   Active workers: {active_count}")
                logger.info(f"[MAIN THREAD]   Max workers: 3")
                logger.info(f"[MAIN THREAD]   Available capacity: {3 - active_count}")

                # Submit message processing to thread pool instead of blocking
                logger.info(f"[MAIN THREAD] ➤ Submitting message #{message_count} to thread pool...")
                message_metadata = {
                    'topic': message.topic,
                ...
                submit_duration = (submit_complete_timestamp - submit_timestamp).total_seconds()

                logger.info(f"[MAIN THREAD] ✓ Message #{message_count} submitted successfully")
                logger.info(f"[MAIN THREAD]   Submission took: {submit_duration*1000:.2f}ms")
                logger.info(f"[MAIN THREAD]   Active workers now: {len(self.active_futures)}")
                logger.info(f"[MAIN THREAD]   Worker thread will process message in background")
                logger.info(f"[MAIN THREAD]   Main thread is now FREE to poll Kafka again")
                logger.info("█"*80)

                # Periodically clean up completed futures to avoid memory growth
                if message_count % 10 == 0:
                    logger.info(f"[MAIN THREAD] Periodic cleanup triggered (every 10 messages)")
                    self._cleanup_completed_futures()
                elif message_count % 5 == 0:
                    logger.info(f"[MAIN THREAD] Thread pool status: {len(self.active_futures)} active workers")

                logger.info(f"[MAIN THREAD] Ready to receive next message (total processed so far: {message_count})\n")
            ...
            logger.info(f"[SHUTDOWN] Setting shutdown event to stop worker threads...")
            self.shutdown_event.set()
            logger.info(f"[SHUTDOWN] Shutdown event set")

            # Wait for in-flight message processing to complete
            in_flight_count = len(self.active_futures)
            if in_flight_count > 0:
                logger.info(f"[SHUTDOWN] ⏳ Waiting for {in_flight_count} in-flight message(s) to complete...")
                logger.info(f"[SHUTDOWN] Each message has 30 seconds timeout")
                for i, future in enumerate(self.active_futures, 1):
                    logger.info(f"[SHUTDOWN] Waiting for message {i}/{in_flight_count}...")
                    try:
                        future.result(timeout=30)  # Wait up to 30 seconds per message
                        logger.info(f"[SHUTDOWN] ✓ Message {i}/{in_flight_count} completed successfully")
                    except TimeoutError:
                        logger.error(f"[SHUTDOWN] ✗ Message {i}/{in_flight_count} timed out after 30 seconds")
                    except Exception as e:
                        logger.error(f"[SHUTDOWN] ✗ Message {i}/{in_flight_count} failed: {e}")
                logger.info(f"[SHUTDOWN] ✓ All in-flight messages processed")
            else:
                logger.info(f"[SHUTDOWN] No in-flight messages to wait for")
            ...
            logger.info("[SHUTDOWN]   2/3: Flushing Kafka producer (ensuring all messages sent)...")
            self.producer.flush()
