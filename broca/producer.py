    ...
    def send_message(self, text):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.info(f"Message sent: {text[:50]}... (offset: {record_metadata.offset})")
                return True

            except InvalidTimestampError as e:
                logger.warning(f"Timestamp error on attempt {attempt + 1}/{self.max_retries}: {e}")
                if attempt < self.max_retries - 1:
                    ...
                    logger.error(f"Failed to send message after {self.max_retries} attempts due to timestamp issues")
                    return False
